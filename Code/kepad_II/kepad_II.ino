...
Adafruit_I2C_Keypad customKeypad = Adafruit_I2C_Keypad( makeKeymap(keys), MCP23017_PORT_A, MCP23017_PORT_B, ROWS, COLS, &mcp);
...
void loop() {
  ...
  while (customKeypad.available()) {        //If a new push button was detected...
    ...
    if (SHIFT && e.bit.KEY == 0x02) { // Cap Lock Key
      CAP_LOCK = !CAP_LOCK;
      digitalWrite(RED_PIN, CAP_LOCK);
      continue;
    }

    if (SHIFT && e.bit.KEY == 0x03) { // Num Lock Key
      NUM_LOCK = !NUM_LOCK;
      digitalWrite(YEL_PIN, NUM_LOCK);
      continue;
    }
  ...
  delay(5); //Small belay between each loop
}
