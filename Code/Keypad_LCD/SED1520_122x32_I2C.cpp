...
SED1520_122x32_I2C::SED1520_122x32_I2C(Adafruit_MCP23017 *mcp) : SED1520() {
  _mcp = mcp;
}

SED1520_122x32_I2C::~SED1520_122x32_I2C() {

}

void SED1520_122x32_I2C::WaitForStatus(unsigned char status, unsigned char controller) {
  char tmp;
...
void SED1520_122x32_I2C::WriteCommand(unsigned char commandToWrite, unsigned char ctrl) {
...
void SED1520_122x32_I2C::WriteData(unsigned char dataToWrite) {
...
unsigned char SED1520_122x32_I2C::ReadData() {
  unsigned char tmp;
...
void SED1520_122x32_I2C::InitDisplay() {
  _mcp->portMode(SED1520_DATA_PORT, OUTPUT);
  _mcp->portMode(SED1520_CONTROL_PORT, OUTPUT);
  GetControl();
}

unsigned char SED1520_122x32_I2C::GetControl() {
  control_pins = _mcp->readGPIO(SED1520_CONTROL_PORT);
  return control_pins;
}

void SED1520_122x32_I2C::SetControl() {
  SetControl(control_pins);
}

void SED1520_122x32_I2C::SetControl(unsigned char ctrl) {
  control_pins = ctrl;
